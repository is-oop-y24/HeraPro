...
namespace BackupsExtra.Database
{
    public class LocRepository : IRepository
    {
        ...
        internal LocRepository(string path, string nameOfRestorePoints = "RestorePoint")
        {
        ...
        internal LocRepository(IEnumerable<RestorePoint> context)
        {
        ...
        public void ExtractRestorePoint(RestorePoint rp, string path, string toLocation = null)
        {
            if (!Directory.Exists(path)) return;
            string[] dirs = Directory.GetFiles(path);
            if (toLocation is not null)
            {
                foreach (string zip in dirs)
                {
                    ZipFile.ExtractToDirectory(zip, toLocation);
                }

                return;
            }

            DirectoryInfo tmpDir = Directory.CreateDirectory(path + @"\tmp");
            foreach (string zip in dirs)
            {
                ZipFile.ExtractToDirectory(zip, tmpDir.FullName);
            }

            FileInfo[] files = tmpDir.GetFiles();
            foreach (Storage zip in rp.ZipFiles)
            {
                foreach (FileInfo file in files)
                {
                    foreach (string pathToExtract in zip.Directory.Where(pathToExtract => file.Name == pathToExtract[pathToExtract.LastIndexOf(@"\", StringComparison.Ordinal) ..]))
                    {
                        file.MoveTo(pathToExtract);
                    }
                }
            }
        }
